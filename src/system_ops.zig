...
pub const SystemOps = struct {
    ...
    pub fn getCpuLoad(self: *SystemOps) !u8 {
        const file = try std.Io.Dir.openFileAbsolute(self.io, "/proc/stat", .{});
        defer file.close(self.io);
    ...
    pub fn getMemory(self: *SystemOps) !u8 {
        const file = try std.Io.Dir.openFileAbsolute(self.io, "/proc/meminfo", .{});
        defer file.close(self.io);
    ...
    pub fn getDiskTemp(self: *SystemOps) !u32 {
        ...
        while (i < 10) : (i += 1) {
            ...
            const name_file = std.Io.Dir.openFileAbsolute(self.io, name_path, .{}) catch continue;
            defer name_file.close(self.io);

            var name_buf: [64]u8 = undefined;
            const name_len = name_file.readPositionalAll(self.io, &name_buf, 0) catch continue;
            const name = std.mem.trim(u8, name_buf[0..name_len], &std.ascii.whitespace);

            if (std.mem.find(u8, name, "nvme") != null) {
    ...
    pub fn getUptime(self: *SystemOps) !struct { days: u32, hours: u32, minutes: u32 } {
        const file = try std.Io.Dir.openFileAbsolute(self.io, "/proc/uptime", .{});
        defer file.close(self.io);
        ...
        const dot_pos = std.mem.find(u8, uptime_str, ".") orelse uptime_str.len;
